...
describe SyncRequest do
  ...
  describe ".prioritized" do
    it "orders with highest priority first" do
      ...
      expect(SyncRequest.prioritized).to eq [f,s,t,l]
    end
  end
  ...
  describe "on create" do
    it "defaults priority to 0" do
      ...
      sr.save
      expect(sr.reload.priority).to eq 0
    end
    it "defaults state to :ready" do
      sr = build(:sync_request)
      expect(sr).to be_valid
      sr.save
      expect(sr.reload).to be_ready # ready? == truthy
    end
  ...
  describe "#start" do
    ...
    describe "if exception is raised" do
      before do
        stub_const("SyncRequest::RETRIES",0)
        allow(sync_request).to receive(:syncable_month?).and_raise 'hell'
      end
      it "catches it" do
      ...
      it "sets :state to 'failed'" do
        sync_request.start
        expect(sync_request.state).to eq 'failed'
      end
    end
  ...
  describe "#nigh_only?" do
    ...
    it "returns false for priority 5 job" do
      ...
      expect(sync_request).to_not be_night_only
    end
