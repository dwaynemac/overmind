...
describe SyncRequest do
  ...
  describe "#start" do
    ...
    it "should call school.sync_year_stats(year)" do
      sync_request.school.should_receive(:sync_year_stats).with(sync_request.year)
      sync_request.start
    end
    it "should set state to finished if sync successfull" do
      sync_request.school.should_receive(:sync_year_stats).with(sync_request.year).and_return(true)
    ...
    it "should set state to failed if sync fails" do
      sync_request.school.should_receive(:sync_year_stats).with(sync_request.year).and_return(false)
      sync_request.start
